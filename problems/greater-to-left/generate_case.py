...
def sorted_case(n, ai):
    ...
    return "%d %d\n%s\n%s\n" % (
            n,
            q,
            " ".join(str(s) for s in sorted(random.randint(1, ai) for _ in xrange(n))),
            "\n".join(str(random.randint(1, n)) for _ in xrange(q))
    )

def reversed_case(n, ai):
    n = random.randint(1, n)
    q = random.randint(1, n)
    ai = random.randint(1, ai)
    return "%d %d\n%s\n%s\n" % (
            n,
            q,
            " ".join(str(s) for s in reversed(sorted(random.randint(1, ai) for _ in xrange(n)))),
            "\n".join(str(random.randint(1, n)) for _ in xrange(q))
    )
...
for n, ai in [
    ...
    for _ in xrange(3):
        ...
        fh = open(ifname, 'w+')
        if _%3 == 0:
            test_data = random_case(n, ai)
        else :
            test_data = sorted_case(n, ai)
        fh.write(test_data)
        fh.close()
